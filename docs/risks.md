...
| GMI API key invalid / missing at judge time | **OPEN (Phase 3):** `/api/papers/:id/ask` returns 502 with the upstream error when GMI rejects the key. The Ask e2e is tolerant (it accepts answer OR error), but the demo needs a working GMI_API_KEY. | 3, 7 |
