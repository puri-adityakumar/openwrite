...
| GMI API key invalid / missing at judge time | **MITIGATED (Phase 3):** GMI Cloud provider configured with the Anthropic `/v1/messages` API format at `https://api.gmi-serving.com/v1`, model `MiniMaxAI/MiniMax-M3`. `lib/gmi.ts` posts to `/v1/messages` with `x-api-key` + `anthropic-version: 2023-06-01`; response is normalised to `{ answer, usage }`. End-to-end probe: `POST /api/papers/:id/ask` returns 200 with a grounded answer and `totalTokens: 142`. The e2e Ask test asserts an answer renders on both chromium and judge-ipad. | 3, 7 |
